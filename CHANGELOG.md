...
### Changed
 * Fix `impl Sub` for `Quantity` to be implemented in terms of `-` instead of `+`.
 ...
 * Change `impl Debug` for `Quantity` to use precision information. Previously the implementation
   checked the `alternate` flag which has no affect on printing raw floats.

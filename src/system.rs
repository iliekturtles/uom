...
///
/// An example invocation is given below for a meter-kilogram-second system. Use the `#[macro_use]`
/// attribute when including the `uom` crate to make the `system!` macro available. Macros use a
/// global namespace. Conflicts with macros generated by the `system!` macro for the `ISQ` can be
/// avoided by only including the necessary macros in the `#[macro_use()]` attribute.
///
/// ```
...
macro_rules! system {
    ...
    ) =>
    {
        ...
        /// module `mks` with a system of quantities name `Q`. The `uom` crate needs to be included
        /// with the `#[macro_use]` attribute. The optional units parameter to change the base units
        ...
        macro_rules! $quantities {
            ($path:path, $V:ty) => {
                system!(@quantities $path, $V, $units; $($quantity),+);
            };
            ($path:path, $V:ty, $U:tt) => {
                system!(@quantities $path, $V; $($name),+; $U; $($quantity),+);
            };
        }
    };
    (@quantities $path:path, $V:ty, $U:ident; $($quantity:ident),+) => {
        use $path as system;

        $($quantity!(system::$U<$V>, $V);)+
    };
    (@quantities $path:path, $V:ty; $($name:ident),+; ($($U:ident),+); $($quantity:ident),+) => {
        use $path as system;
...
/// system. Use the `#[macro_use]` attribute when including the `uom` crate to make the `quantity!`
/// macro available. Macros use a global namespace. Conflicts with macros generated by the
/// `quantity!` macro for the `ISQ` can be avoided by only including the necessary macros in the
/// `#[macro_use()]` attribute.
///
/// ```ignore
/// #[macro_use(quantity, system)]
